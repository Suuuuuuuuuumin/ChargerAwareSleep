...
struct PolicyTests {
    static func main() {
        ...
        // 배터리 + 항상 켬일 때만 경고한다
        assert(shouldWarnAlwaysOnBattery(mode: .alwaysOn, source: .battery) == true,
               "되돌리는 걸 잊으면 가방 안에서 안 자고 방전·발열이 난다")
        assert(shouldWarnAlwaysOnBattery(mode: .alwaysOn, source: .ac) == false,
               "충전 중이면 방전 위험이 없어 경고할 이유가 없다")
        assert(shouldWarnAlwaysOnBattery(mode: .auto, source: .battery) == false)
        assert(shouldWarnAlwaysOnBattery(mode: .alwaysOff, source: .battery) == false)
