...
**배터리 + 항상 켬이면 메뉴 맨 위에 경고 한 줄을 띄운다.** 모드를 자동으로
바꾸지 않는다. 사용자가 의도적으로 켠 모드를 앱이 마음대로 끄면, 뚜껑을 잠깐
덮었다 여는 상황(발표 등)에서 더 나쁜 오작동이 된다. 목표는 인지이지 자동
교정이 아니다. 되돌리는 수단은 바로 아래 모드 목록이라 버튼을 따로 두지 않는다.

**한계**: 메뉴를 열어야만 보인다. 안 열어보면 여전히 모른다. 원래 노렸던 건
뚜껑을 열 때 저절로 뜨는 말풍선이었으나 아래 이유로 접었다.

#### 폐기: 아이콘에 붙는 NSPopover 말풍선 (2026-09-10 실측)

- **증상**: 팝오버가 아이콘이 아니라 화면 왼쪽 끝(x=0)에 뜬다.
- **원인**: macOS 26 은 메뉴바를 out-of-process 로 그린다. 앱이 가진
  `NSStatusItem` 의 창은 화면 밖에 주차된 그림자다.
...
- **시도했으나 모두 같은 결과**: `MenuBarExtra` 의 상태아이템 / 직접 만든
  `NSStatusItem` / 번들 `.app`(ad-hoc 서명) / 맨 실행파일 / 배치 지연 8초까지.
  좌표는 8초 내내 화면 밖에 고정이었다.
- **남은 길**: 접근성 API(`AXUIElement`)로 메뉴바 항목의 실제 좌표를 읽는 것.
  접근성 권한이 필요해 설치 절차가 늘어난다 — 그만한 값어치는 없다고 봤다.
- **최종 결정**: 앵커를 포기하고 메뉴 안의 경고 한 줄로 간다.
...
단일 실행 파일. `Sources/` 파일 5개.

상한은 원래 4개였다. 뚜껑 개폐 구독(`LidWatcher`)이 수명주기를 가진 구독
객체라 `App.swift` 에 합치면 `Controller` 가 정책과 구독을 한꺼번에 떠안아
5개가 됐다.
