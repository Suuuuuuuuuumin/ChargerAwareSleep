...
- 첫 줄은 현재 `SleepDisabled` 값을 그대로 보여준다. 앱이 믿는 값이 아니라
  IORegistry에서 읽은 실제 값이다.
- 전원 공급원(충전 중/배터리)은 보여주지 않는다 (2026-09-10 제거). 메뉴바에도
  배터리 표시가 이미 있어 같은 정보가 두 번 뜰 뿐이었다. 판정에는 계속 쓴다.
...
로그인 항목 등록·해제는 앱이 하지 않는다. `SMAppService` 로 메뉴에서 켜고 끄게
했었으나 (2026-09-10 제거) 한 번만 정하면 되는 설정이라 메뉴만 어지럽혔다.
시스템 설정 > 일반 > 로그인 항목 이 같은 일을 하고, 그쪽이 사용자가 이미 아는
자리다. ad-hoc 서명 앱도 여기서 정상 등록·기동된다 (2026-09-07 재부팅 실측).


```bash
sudo -n /usr/bin/pmset -a disablesleep 1
ioreg -n IOPMrootDomain -r -d1 | grep -o '"SleepDisabled" = [A-Za-z]*'   # Yes 확인
# 앱 종료 + 시스템 설정 > 일반 > 로그인 항목 에서 해제 + 재부팅
