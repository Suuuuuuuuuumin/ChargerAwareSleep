...
### 항상 켬 유지 확인 (2026-09-10 추가)

"항상 켬"은 전원 공급원을 무시하므로, 되돌리는 걸 잊은 채 배터리로 가방에
넣으면 종료 시 복원(아래)이 걸리지 않는 한 안 자고 방전·발열이 난다. 실제로
겪은 사고다 — 자리를 옮기려고 켜두고 도착지에서 잊었다.

**배터리 + 항상 켬 상태로 뚜껑이 열리면** 메뉴바 아이콘에 붙는 5초짜리
`NSPopover` 로 유지 여부를 묻는다. 응답이 없으면 모드를 유지한 채 닫는다.

- 트리거가 뚜껑 **열림**인 이유: 닫는 순간엔 화면을 볼 사람이 없다. 다시 열어
  자리에 도착한 시점이 사용자가 화면 앞에 있다고 확신할 수 있는 유일한 순간이다.
- 배터리일 때만 묻는 이유: 충전 중이면 방전·발열 위험이 없다. 항상 켬이 의도한
  대로 동작하는 상황이라 물어볼 이유가 없다.
- **자동으로 모드를 바꾸지 않는다.** 사용자가 의도적으로 켠 모드를 앱이 마음대로
  끄면, 뚜껑을 잠깐 덮었다 여는 상황(발표 등)에서 더 나쁜 오작동이 된다.
  목표는 인지이지 자동 교정이 아니다.

앵커는 `NSApp.windows` 안의 `NSStatusBarWindow` 다. `MenuBarExtra` 의
`NSStatusItem` 을 밖에서 잡을 수 있는지 프로브로 실측 확인했다 (2026-09-08:
`frame={{-3654,1290},{37,39}}`, `contentView`=`NSStatusBarContentView`, 팝오버
표시 후 4초 뒤에도 유지). 못 찾으면 조용히 포기한다 — 오류 필드를 하나 더
늘려 메뉴를 어지럽히는 것보다, 뜨지 않아도 핵심 동작(잠자기 제어)이 멀쩡한
쪽을 택했다.

**미측정**: 뚜껑을 여는 순간은 디스플레이가 웨이크 중이라 상태바 창이 아직
준비 안 됐을 수 있어 0.4초 늦춰 앵커를 찾는다. 이 값은 추정치다 — 말풍선이
안 뜨면 여기부터 키운다.
...
| 항상 켬 확인 말풍선 | `NSPopover` + `NSHostingController`, 앵커는 `NSApp.windows` 의 `NSStatusBarWindow` |
